#line 1 "Source\\PtsPta_Protection.c"
...
#line 13 "Source\\PtsPta_Protection.c"
...
#line 14 "Source\\PtsPta_Protection.c"
...
#line 15 "Source\\PtsPta_Protection.c"
...
#line 16 "Source\\PtsPta_Protection.c"
...
#line 17 "Source\\PtsPta_Protection.c"
#line 18 "Source\\PtsPta_Protection.c"
...
void PCB_Temperature_Check(void)
{
	...
	if(G_PCBStatus == ePCBStatus_Normal) 
	{
		if(G_NTC_PCB > 3510)
		{				
			S_TempOver_80_Cnt++;
			
			if(S_TempOver_80_Cnt >= (400))
			{
...
void Motor_Temperature_Check(void)
{
	...
	if(G_MotorStatus == eMotorStatus_Normal) 
	{
		if(G_NTC_Motor >= 3734)
		{			
			printf("Motor Temp=%d,\n",S_TempOver_100_Cnt);
	...
	else if(G_MotorStatus == eMotorStatus_Over100)
	{

		if(G_NTC_Motor > 3638 && G_NTC_Motor < 3734 && G_BuzzerLED == eBehaving_OFF)
		{
	...
	else if(G_MotorStatus == eMotorStatus_Over90_100) 
	{		
		if(G_NTC_Motor < 3638)
			S_TempUnder_90_Cnt ++;
	
		if(S_TempUnder_90_Cnt >= (600))			
		{
			G_MotorStatus = eMotorStatus_Normal;
			G_BuzzerLED = eBehaving_OFF;
...
void OverCurrent_Check(void)
{
	...
	if(G_Current >= 2300) 
	{
		if(S_OverCurrentOverLoadCnt > (240/5)) 
		{
			Motor(0, G_Direction);
			printf("I2\n");
			G_BuzzerLED = eBehaving_CurrentOver;  
			G_MotorStatus = eMotorStatus_OverCurrent;
			S_OverCurrentOverLoadCnt = (240/5);
	...
	if(G_Current >= 1400) 
	{
		if(S_OverCurrentProtectCnt > (3600/5)) 
		{
			Motor(0, G_Direction);
			printf("I3\n");
			G_BuzzerLED = eBehaving_CurrentOver;  	
			G_MotorStatus = eMotorStatus_OverCurrent;
			S_OverCurrentProtectCnt = (3600/5);
		}
	}
	else
	{
...
void BatteryNTC_Check(void)
{
	...
	if(G_BatteryStatus == eBatteryStatus_Normal)
	{
		...
		if(G_NTC_Battery <  110 || G_NTC_Battery >  360) 
		{
			S_BatteryErrorCnt ++;
			
			if(S_BatteryErrorCnt > (500/5)) 
			{
				G_BuzzerLED = eBehaving_BatteryNTCErr;
				G_BatteryStatus = eBatteryStatus_Error;
				Motor(0, G_Direction);
				printf("B-NTC Err = %d\n",G_NTC_Battery);
				S_BatteryErrorCnt = (500/5);
			}
		}
		else
		{
	...
	else if(G_BatteryStatus == eBatteryStatus_Error)
	{
		...
		{
			S_BatteryNormalCnt ++;	
			if(S_BatteryNormalCnt > (500/5))
			{
				G_BatteryStatus = eBatteryStatus_Normal;

				printf("B-NTC OK\n");
				S_BatteryNormalCnt = (500/5);
			}
